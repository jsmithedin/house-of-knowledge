...
def test_tracer_trace_sdk_failure_returns_noop_trace():
    ...
    mock_client.start_observation.side_effect = Exception("network error")
...
def test_trace_span_sdk_failure_returns_noop_obj():
    mock_root = MagicMock()
    mock_root.start_observation.side_effect = Exception("error")
    trace = _Trace(mock_root, MagicMock())
...
def test_generation_end_sdk_failure_does_not_raise():
    mock_gen = MagicMock()
    mock_gen.update.side_effect = Exception("flush error")
    gen = _Generation(mock_gen)
    gen.end(output="answer", input_tokens=1, output_tokens=1)  # must not raise


def test_trace_end_calls_set_trace_io_end_and_flush():
    mock_root = MagicMock()
    mock_client = MagicMock()
    trace = _Trace(mock_root, mock_client)
    trace.end("final output")
    mock_root.set_trace_io.assert_called_once_with(output="final output")
...
def test_trace_end_sdk_failure_does_not_raise():
    mock_root = MagicMock()
    mock_root.set_trace_io.side_effect = Exception("error")
    trace = _Trace(mock_root, MagicMock())
